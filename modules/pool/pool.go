...
// the Pool.
func newPool(dependencies dependencies, cs modules.ConsensusSet, tpool modules.TransactionPool, gw modules.Gateway, wallet modules.Wallet, persistDir string, initConfig PoolConfig) (*Pool, error) {
	...
	p.log, err = dependencies.newPoolLogger(initConfig.PoolLogDir)
	if err != nil {
		return nil, err
	}
...
func New(cs modules.ConsensusSet, tpool modules.TransactionPool, gw modules.Gateway, wallet modules.Wallet, persistDir string, initConfig PoolConfig) (*Pool, error) {
...
func (p *Pool) coinB1Txn() string {
	...
	MarshalSiaNoSignatures(coinbaseTxn, buf)
	b := buf.Bytes()
...
func (p *Pool) startHttp() {
	http.HandleFunc("/getblocktmp", func(writer http.ResponseWriter, request *http.Request) {
		getBlockTmp(writer, request, p)
	})
	http.HandleFunc("/submit", func(writer http.ResponseWriter, request *http.Request) {
		submit(writer, request, p)
	})
	http.ListenAndServe(":8088", nil)
}

func getBlockTmp(w http.ResponseWriter, r *http.Request, p *Pool) {
	if !p.cs.Synced() {
		api.WriteJSON(w, ConsensusNotify{Error: "chain not synced"})
		return
	}
	finalBlock := p.cs.CurrentBlock()
	parentID := finalBlock.ID()
	...
	nbits := fmt.Sprintf("%08x", BigToCompact(p.persist.Target.Int()))

	var buf bytes.Buffer
	encoding.WriteUint64(&buf, uint64(p.sourceBlock.Timestamp))
	ntime := hex.EncodeToString(buf.Bytes())

	api.WriteJSON(w, ConsensusNotify{
		Target:    p.persist.Target,
		Height:    p.cs.Height() + 1,
		Block:     p.sourceBlock,
...
func submit(w http.ResponseWriter, r *http.Request, p *Pool) {
	// 需要解析r的数据，找到block
	var b types.Block
	var r1 = make(map[string]types.Block)
	...
	// 打印
	if err != nil {
		api.WriteJSON(w, SubmitResponse{
			Message: err.Error(),
		})
	}
